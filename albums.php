  ...
  <?php
    ...
    if (mysqli_num_rows($select_album_result) > 0) {
        ...
        while($row = mysqli_fetch_assoc($select_album_result)) {
            ...
            <td><a href = "editAlbum.php?ed=<?php echo $row['cdID'] . "&title=" . $row['cdTitle'] . "&price=" . $row['cdPrice'] . "&genre=" . $row['cdGenre'] . "&artist=" . $row['artName'] . "&tracks=" . $row['cdNumTracks']?>" class = "editButton">Edit</a></td>
